...
import { deleteCatalog } from './actions'; // Impor fungsi delete

export default function CatalogTable({ catalogs }) {
  const [isPending, startTransition] = useTransition();

  // === PERUBAHAN: Modifikasi handleDelete ===
  const handleDelete = (catalog) => {
    const { id, title, metadata } = catalog;
    
    // Ambil nama tabel dari metadata (ini sumber yang paling bisa diandalkan)
    const tableName = metadata?.nama_tabel_database;
    
    if (window.confirm(`Apakah Anda yakin ingin menghapus katalog "${title}"? Ini akan menghapus data dari 'datasets' DAN tabel data fisik '${tableName || ''}' secara permanen.`)) {
      startTransition(async () => {
        // Teruskan ID dan nama tabel yang akan dihapus
        const result = await deleteCatalog(id, tableName); 
        if (result.success) {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {catalogs.length > 0 ? (
              catalogs.map((catalog) => ( // 'catalog' adalah objek utuh
                <tr key={catalog.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                    ...
                    {/* === PERUBAHAN: Kirim seluruh objek 'catalog' === */}
                    <button 
