...
console.log("Edge Function 'process-csv-upload' dimuat (Versi Perbaikan 5 - Fix RLS)."); // Versi update
...
      throw new Error('SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, atau SUPABASE_ANON_KEY tidak diatur.');
    }
...
    console.log("Tabel berhasil dibuat.");
    ...
    // --- REVISI: Pisahkan perintah RLS ---
    console.log(`Menerapkan RLS (Langkah 1/2): ALTER TABLE...`);
    const { error: rlsEnableError } = await supabaseAdmin.rpc('exec', { 
      sql_query: `ALTER TABLE public."${tableName}" ENABLE ROW LEVEL SECURITY;`
    });
    if (rlsEnableError) {
        console.error("Error saat menerapkan RLS (ALTER TABLE):", rlsEnableError);
        throw new Error(`Tabel dibuat, tapi gagal mengaktifkan RLS: ${rlsEnableError.message}`);
    }

    console.log(`Menerapkan RLS (Langkah 2/2): CREATE POLICY...`);
    // Buat nama policy yang aman dan unik
    const policyName = `Public_Read_${tableName}`.slice(0, 63); // Batas 63 karakter
    const { error: rlsPolicyError } = await supabaseAdmin.rpc('exec', { 
      sql_query: `CREATE POLICY "${policyName}" ON public."${tableName}" FOR SELECT USING (true);`
    });
    if (rlsPolicyError) {
        console.error("Error saat menerapkan RLS (CREATE POLICY):", rlsPolicyError);
        throw new Error(`RLS diaktifkan, tapi gagal membuat policy: ${rlsPolicyError.message}`);
  Dibuat }
...
    }
    
    const apiUrl = `${supabaseUrl}/rest/v1/${tableName}?select=*&apikey=${anonKey}`;
    console.log(`API URL publik dibuat: ${apiUrl}`);
...
    
    // Rollback
    if (tableName && (err as Error).message.includes("Gagal memasukkan data") || (err as Error).message.includes("gagal menerapkan RLS") || (err as Error).message.includes("gagal membuat policy")) {
        console.log(`Mencoba menghapus tabel ${tableName} karena proses insert/RLS gagal...`);
        const supabaseAdmin = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
        await supabaseAdmin.rpc('exec', { sql_query: `DROP TABLE IF EXISTS public."${tableName}";` });
        console.log(`Tabel ${tableName} berhasil dihapus (rollback).`);
    }
